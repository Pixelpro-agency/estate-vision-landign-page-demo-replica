...
function IntelligenceLabel({ position, label, type }: { position: [number, number, number], label: string, type: 'risk' | 'source' | 'asset' | 'grid' }) {
    return (
        <Html position={position} distanceFactor={15} zIndexRange={[100, 0]} style={{ pointerEvents: 'none' }}>
            ...
            `}>
                <div className={`w-2 h-2 rounded-full animate-pulse ${type === 'risk' ? 'bg-red-500 shadow-[0_0_10px_#ef4444]' :
    ...
    return (
        <group ref={groupRef} position={[0, -2.5, 0]}>
            ...
            <group position={[1, 0, -1]}>
                ...
                <Text position={[0, 6.5, 1.28]} fontSize={0.35} color="#60a5fa" fontWeight="900" letterSpacing={0.15}>NOVA STRATOS</Text>
                <Box args={[3.2, 0.7, 0.05]} position={[0, 6.5, 1.25]}>
            ...
            {/* --- INFRASTRUCTURE --- */}
            {/* Moved roads further out and lowered them slightly to avoid building intersection and z-fighting */}
            <RealisticRoad position={[0, 0.01, 5.5]} length={14} />
            <RealisticRoad position={[-6.5, 0.01, 0]} length={12} rotation={[0, Math.PI / 2, 0]} />
            ...
            <group position={[3, 1.5, 3]}>
                ...
                {/* Adjusted to follow new road position */}
                <DataConnection start={[4, 0, 5.5]} end={[2, 1, 2]} color="#ef4444" />
            </group>
...
export default function AssetScene() {
    return (
        ...
        >
            <Canvas camera={{ position: [8, 8, 14], fov: 30 }} dpr={[1, 2]}>
