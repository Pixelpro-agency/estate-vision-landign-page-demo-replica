/**
 * Parcels M06 Layer Configuration
...
export class ParcelsM06Layer {
  constructor() {
    this.layerName = 'parcels-m06';
    this.layerState = true; // Default visible state
    this.layerInToc = true; // Include in Table of Contents
    this.layerColor = '#32CD32'; // Lime green color for agriculture
    this.pmtilesUrl =
      'https://storage.googleapis.com/space-neural-02/tiles/pmtiles/pg_parcels_251001_m06.pmtile';
    this.sourceLayer = 'default'; // Default source layer, will be updated from metadata
  }

  /**
   * Add the parcels-m06 layer to a MapLibre GL map
  ...
  async addToMap(map) {
    try {
      ...
      map.on('sourcedata', e => {
        if (e.sourceId === 'parcels-m06' && e.isSourceLoaded) {
          // _onsole.log('✓ Parcels M06 PMTiles source loaded successfully');
        }
      });

      map.on('error', e => {
        if (e.error && e.error.message.includes('parcels-m06')) {
          console.error(' Parcels M06 layer error:', e.error);
        }
      });
      ...
      if (toggleButton) {
        ...
        if (titleSpan) {
          titleSpan.textContent = 'AGRICOLTURA';
        }
      ...
      // _onsole.log('✓ Parcels M06 PMTiles layer added successfully');
    } catch (error) {
      console.error(' Failed to add parcels-m06 PMTiles layer:', error);
    }
  }
  ...
  setVisibility(map, visible) {
    ...
    if (map.getLayer('parcels-m06-fill')) {
      map.setLayoutProperty('parcels-m06-fill', 'visibility', visibility);
    }
    if (map.getLayer('parcels-m06-highlight')) {
      map.setLayoutProperty('parcels-m06-highlight', 'visibility', visibility);
    }
    if (map.getLayer('parcels-m06-selected')) {
      map.setLayoutProperty('parcels-m06-selected', 'visibility', visibility);
    }
  }

  /**
   * Show the parcels-m06 layers
   * @param {Map} map - MapLibre GL map instance
   */
  showLayers(map) {
  ...
  /**
   * Check if the parcels-m06 layers are currently visible
   * @param {Map} map - MapLibre GL map instance
   * @returns {boolean} True if layers are visible
   */
  isVisible(map) {
    const fillLayer = map.getLayer('parcels-m06-fill');
    if (fillLayer) {
  ...
  setLayerColor(newColor, map = null) {
    ...
    if (map && map.getLayer('parcels-m06-fill')) {
      map.setPaintProperty('parcels-m06-fill', 'fill-color', this.layerColor);
    }
  }
