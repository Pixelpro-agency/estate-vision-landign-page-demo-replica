/**
 * Parcels M00 Layer Configuration
...
export class ParcelsM00Layer {
  constructor() {
    this.layerName = 'parcels-m00';
    this.layerState = true; // Default visible state
    this.layerInToc = true; // Include in Table of Contents
    this.layerColor = '#808080'; // Gray color for N/A parcels
    this.pmtilesUrl =
      'https://storage.googleapis.com/space-neural-02/tiles/pmtiles/pg_parcels_251001_m00.pmtile';
    this.sourceLayer = 'default'; // Default source layer, will be updated from metadata
  }

  /**
   * Add the parcels-m00 layer to a MapLibre GL map
  ...
  async addToMap(map) {
    try {
      ...
      map.on('sourcedata', e => {
        if (e.sourceId === 'parcels-m00' && e.isSourceLoaded) {
          // _onsole.log('✓ Parcels M00 PMTiles source loaded successfully');
        }
      });

      map.on('error', e => {
        if (e.error && e.error.message.includes('parcels-m00')) {
          console.error(' Parcels M00 layer error:', e.error);
        }
      });
      ...
      // _onsole.log('✓ Parcels M00 PMTiles layer added successfully');
    } catch (error) {
      console.error(' Failed to add parcels-m00 PMTiles layer:', error);
    }
  }
  ...
  setVisibility(map, visible) {
    ...
    if (map.getLayer('parcels-m00-fill')) {
      map.setLayoutProperty('parcels-m00-fill', 'visibility', visibility);
    }
    if (map.getLayer('parcels-m00-highlight')) {
      map.setLayoutProperty('parcels-m00-highlight', 'visibility', visibility);
    }
    if (map.getLayer('parcels-m00-selected')) {
      map.setLayoutProperty('parcels-m00-selected', 'visibility', visibility);
    }
  }

  /**
   * Show the parcels-m00 layers
   * @param {Map} map - MapLibre GL map instance
   */
  showLayers(map) {
  ...
  /**
   * Check if the parcels-m00 layers are currently visible
   * @param {Map} map - MapLibre GL map instance
   * @returns {boolean} True if layers are visible
   */
  isVisible(map) {
    const fillLayer = map.getLayer('parcels-m00-fill');
    if (fillLayer) {
  ...
  setLayerColor(newColor, map = null) {
    ...
    if (map && map.getLayer('parcels-m00-fill')) {
      map.setPaintProperty('parcels-m00-fill', 'fill-color', this.layerColor);
    }
  }
