/**
 * Parcels M01 Layer Configuration
...
export class ParcelsM01Layer {
  constructor() {
    this.layerName = 'parcels-m01';
    this.layerState = true; // Default visible state
    this.layerInToc = true; // Include in Table of Contents
    this.layerColor = '#FFD700'; // Gold color for residential areas
    this.pmtilesUrl =
      'https://storage.googleapis.com/space-neural-02/tiles/pmtiles/pg_parcels_251001_m01.pmtile';
    this.sourceLayer = 'default'; // Default source layer, will be updated from metadata
  }

  /**
   * Add the parcels-m01 layer to a MapLibre GL map
  ...
  async addToMap(map) {
    try {
      ...
      map.on('sourcedata', e => {
        if (e.sourceId === 'parcels-m01' && e.isSourceLoaded) {
          // _onsole.log('✓ Parcels M01 PMTiles source loaded successfully');
        }
      });

      map.on('error', e => {
        if (e.error && e.error.message.includes('parcels-m01')) {
          console.error(' Parcels M01 layer error:', e.error);
        }
      });
      ...
      if (toggleButton) {
        ...
        if (titleSpan) {
          titleSpan.textContent = 'AREA RESIDENZIALE';
        }
      ...
      // _onsole.log('✓ Parcels M01 PMTiles layer added successfully');
    } catch (error) {
      console.error(' Failed to add parcels-m01 PMTiles layer:', error);
    }
  }
  ...
  setVisibility(map, visible) {
    ...
    if (map.getLayer('parcels-m01-fill')) {
      map.setLayoutProperty('parcels-m01-fill', 'visibility', visibility);
    }
    if (map.getLayer('parcels-m01-highlight')) {
      map.setLayoutProperty('parcels-m01-highlight', 'visibility', visibility);
    }
    if (map.getLayer('parcels-m01-selected')) {
      map.setLayoutProperty('parcels-m01-selected', 'visibility', visibility);
    }
  }

  /**
   * Show the parcels-m01 layers
   * @param {Map} map - MapLibre GL map instance
   */
  showLayers(map) {
  ...
  /**
   * Check if the parcels-m01 layers are currently visible
   * @param {Map} map - MapLibre GL map instance
   * @returns {boolean} True if layers are visible
   */
  isVisible(map) {
    const fillLayer = map.getLayer('parcels-m01-fill');
    if (fillLayer) {
  ...
  setLayerColor(newColor, map = null) {
    ...
    if (map && map.getLayer('parcels-m01-fill')) {
      map.setPaintProperty('parcels-m01-fill', 'fill-color', this.layerColor);
    }
  }
