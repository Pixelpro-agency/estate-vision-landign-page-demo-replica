/**
 * Parcels M08 Layer Configuration
...
export class ParcelsM08Layer {
  constructor() {
    this.layerName = 'parcels-m08';
    this.layerState = true; // Default visible state
    this.layerInToc = true; // Include in Table of Contents
    this.layerColor = '#228B22'; // Forest green color for natural areas
    this.pmtilesUrl =
      'https://storage.googleapis.com/space-neural-02/tiles/pmtiles/pg_parcels_251001_m08.pmtile';
    this.sourceLayer = 'default'; // Default source layer, will be updated from metadata
  }

  /**
   * Add the parcels-m08 layer to a MapLibre GL map
  ...
  async addToMap(map) {
    try {
      ...
      map.on('sourcedata', e => {
        if (e.sourceId === 'parcels-m08' && e.isSourceLoaded) {
          // _onsole.log('✓ Parcels M08 PMTiles source loaded successfully');
        }
      });

      map.on('error', e => {
        if (e.error && e.error.message.includes('parcels-m08')) {
          console.error(' Parcels M08 layer error:', e.error);
        }
      });
      ...
      if (toggleButton) {
        ...
        if (titleSpan) {
          titleSpan.textContent = 'AREE NATURALI';
        }
      ...
      // _onsole.log('✓ Parcels M08 PMTiles layer added successfully');
    } catch (error) {
      console.error(' Failed to add parcels-m08 PMTiles layer:', error);
    }
  }
  ...
  setVisibility(map, visible) {
    ...
    if (map.getLayer('parcels-m08-fill')) {
      map.setLayoutProperty('parcels-m08-fill', 'visibility', visibility);
    }
    if (map.getLayer('parcels-m08-highlight')) {
      map.setLayoutProperty('parcels-m08-highlight', 'visibility', visibility);
    }
    if (map.getLayer('parcels-m08-selected')) {
      map.setLayoutProperty('parcels-m08-selected', 'visibility', visibility);
    }
  }

  /**
   * Show the parcels-m08 layers
   * @param {Map} map - MapLibre GL map instance
   */
  showLayers(map) {
  ...
  /**
   * Check if the parcels-m08 layers are currently visible
   * @param {Map} map - MapLibre GL map instance
   * @returns {boolean} True if layers are visible
   */
  isVisible(map) {
    const fillLayer = map.getLayer('parcels-m08-fill');
    if (fillLayer) {
  ...
  setLayerColor(newColor, map = null) {
    ...
    if (map && map.getLayer('parcels-m08-fill')) {
      map.setPaintProperty('parcels-m08-fill', 'fill-color', this.layerColor);
    }
  }
