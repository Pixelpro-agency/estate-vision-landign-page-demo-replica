/**
 * Parcels M02 Layer Configuration
...
export class ParcelsM02Layer {
  constructor() {
    this.layerName = 'parcels-m02';
    this.layerState = true; // Default visible state
    this.layerInToc = true; // Include in Table of Contents
    this.layerColor = '#4169E1'; // Royal blue color for public services/administration
    this.pmtilesUrl =
      'https://storage.googleapis.com/space-neural-02/tiles/pmtiles/pg_parcels_251001_m02.pmtile';
    this.sourceLayer = 'default'; // Default source layer, will be updated from metadata
  }

  /**
   * Add the parcels-m02 layer to a MapLibre GL map
  ...
  async addToMap(map) {
    try {
      ...
      map.on('sourcedata', e => {
        if (e.sourceId === 'parcels-m02' && e.isSourceLoaded) {
          // _onsole.log('✓ Parcels M02 PMTiles source loaded successfully');
        }
      });

      map.on('error', e => {
        if (e.error && e.error.message.includes('parcels-m02')) {
          console.error(' Parcels M02 layer error:', e.error);
        }
      });
      ...
      if (toggleButton) {
        ...
        if (titleSpan) {
          titleSpan.textContent = 'SERVIZI PUBBLICI/AMMIN.';
        }
      ...
      // _onsole.log('✓ Parcels M02 PMTiles layer added successfully');
    } catch (error) {
      console.error(' Failed to add parcels-m02 PMTiles layer:', error);
    }
  }
  ...
  setVisibility(map, visible) {
    ...
    if (map.getLayer('parcels-m02-fill')) {
      map.setLayoutProperty('parcels-m02-fill', 'visibility', visibility);
    }
    if (map.getLayer('parcels-m02-highlight')) {
      map.setLayoutProperty('parcels-m02-highlight', 'visibility', visibility);
    }
    if (map.getLayer('parcels-m02-selected')) {
      map.setLayoutProperty('parcels-m02-selected', 'visibility', visibility);
    }
  }

  /**
   * Show the parcels-m02 layers
   * @param {Map} map - MapLibre GL map instance
   */
  showLayers(map) {
  ...
  /**
   * Check if the parcels-m02 layers are currently visible
   * @param {Map} map - MapLibre GL map instance
   * @returns {boolean} True if layers are visible
   */
  isVisible(map) {
    const fillLayer = map.getLayer('parcels-m02-fill');
    if (fillLayer) {
  ...
  setLayerColor(newColor, map = null) {
    ...
    if (map && map.getLayer('parcels-m02-fill')) {
      map.setPaintProperty('parcels-m02-fill', 'fill-color', this.layerColor);
    }
  }
