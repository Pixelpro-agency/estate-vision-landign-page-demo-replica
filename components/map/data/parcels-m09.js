/**
 * Parcels M09 Layer Configuration
...
export class ParcelsM09Layer {
  constructor() {
    this.layerName = 'parcels-m09';
    this.layerState = true; // Default visible state
    this.layerInToc = true; // Include in Table of Contents
    this.layerColor = '#90EE90'; // Light green color for cultural interest areas
    this.pmtilesUrl =
      'https://storage.googleapis.com/space-neural-02/tiles/pmtiles/pg_parcels_251001_m09.pmtile';
    this.sourceLayer = 'default'; // Default source layer, will be updated from metadata
  }

  /**
   * Add the parcels-m09 layer to a MapLibre GL map
  ...
  async addToMap(map) {
    try {
      ...
      map.on('sourcedata', e => {
        if (e.sourceId === 'parcels-m09' && e.isSourceLoaded) {
          // _onsole.log('✓ Parcels M09 PMTiles source loaded successfully');
        }
      });

      map.on('error', e => {
        if (e.error && e.error.message.includes('parcels-m09')) {
          console.error(' Parcels M09 layer error:', e.error);
        }
      });
      ...
      if (toggleButton) {
        ...
        if (titleSpan) {
          titleSpan.textContent = 'INTERESSE COLTURALE';
        }
      ...
      // _onsole.log('✓ Parcels M09 PMTiles layer added successfully');
    } catch (error) {
      console.error(' Failed to add parcels-m09 PMTiles layer:', error);
    }
  }
  ...
  setVisibility(map, visible) {
    ...
    if (map.getLayer('parcels-m09-fill')) {
      map.setLayoutProperty('parcels-m09-fill', 'visibility', visibility);
    }
    if (map.getLayer('parcels-m09-highlight')) {
      map.setLayoutProperty('parcels-m09-highlight', 'visibility', visibility);
    }
    if (map.getLayer('parcels-m09-selected')) {
      map.setLayoutProperty('parcels-m09-selected', 'visibility', visibility);
    }
  }

  /**
   * Show the parcels-m09 layers
   * @param {Map} map - MapLibre GL map instance
   */
  showLayers(map) {
  ...
  /**
   * Check if the parcels-m09 layers are currently visible
   * @param {Map} map - MapLibre GL map instance
   * @returns {boolean} True if layers are visible
   */
  isVisible(map) {
    const fillLayer = map.getLayer('parcels-m09-fill');
    if (fillLayer) {
  ...
  setLayerColor(newColor, map = null) {
    ...
    if (map && map.getLayer('parcels-m09-fill')) {
      map.setPaintProperty('parcels-m09-fill', 'fill-color', this.layerColor);
    }
  }
