/**
 * Parcels M10 Layer Configuration
...
export class ParcelsM10Layer {
  constructor() {
    this.layerName = 'parcels-m10';
    this.layerState = true; // Default visible state
    this.layerInToc = true; // Include in Table of Contents
    this.layerColor = '#00CED1'; // Dark turquoise color for water and wetland areas
    this.pmtilesUrl =
      'https://storage.googleapis.com/space-neural-02/tiles/pmtiles/pg_parcels_251001_m10.pmtile';
    this.sourceLayer = 'default'; // Default source layer, will be updated from metadata
  }

  /**
   * Add the parcels-m10 layer to a MapLibre GL map
  ...
  async addToMap(map) {
    try {
      ...
      map.on('sourcedata', e => {
        if (e.sourceId === 'parcels-m10' && e.isSourceLoaded) {
          // _onsole.log('✓ Parcels M10 PMTiles source loaded successfully');
        }
      });

      map.on('error', e => {
        if (e.error && e.error.message.includes('parcels-m10')) {
          console.error(' Parcels M10 layer error:', e.error);
        }
      });
      ...
      if (toggleButton) {
        ...
        if (titleSpan) {
          titleSpan.textContent = 'ACQUE ZONE UMIDE';
        }
      ...
      // _onsole.log('✓ Parcels M10 PMTiles layer added successfully');
    } catch (error) {
      console.error(' Failed to add parcels-m10 PMTiles layer:', error);
    }
  }
  ...
  setVisibility(map, visible) {
    ...
    if (map.getLayer('parcels-m10-fill')) {
      map.setLayoutProperty('parcels-m10-fill', 'visibility', visibility);
    }
    if (map.getLayer('parcels-m10-highlight')) {
      map.setLayoutProperty('parcels-m10-highlight', 'visibility', visibility);
    }
    if (map.getLayer('parcels-m10-selected')) {
      map.setLayoutProperty('parcels-m10-selected', 'visibility', visibility);
    }
  }

  /**
   * Show the parcels-m10 layers
   * @param {Map} map - MapLibre GL map instance
   */
  showLayers(map) {
  ...
  /**
   * Check if the parcels-m10 layers are currently visible
   * @param {Map} map - MapLibre GL map instance
   * @returns {boolean} True if layers are visible
   */
  isVisible(map) {
    const fillLayer = map.getLayer('parcels-m10-fill');
    if (fillLayer) {
  ...
  setLayerColor(newColor, map = null) {
    ...
    if (map && map.getLayer('parcels-m10-fill')) {
      map.setPaintProperty('parcels-m10-fill', 'fill-color', this.layerColor);
    }
  }
