/**
 * Parcels M07 Layer Configuration
...
export class ParcelsM07Layer {
  constructor() {
    this.layerName = 'parcels-m07';
    this.layerState = true; // Default visible state
    this.layerInToc = true; // Include in Table of Contents
    this.layerColor = '#1E90FF'; // Dodger blue color for water facilities
    this.pmtilesUrl =
      'https://storage.googleapis.com/space-neural-02/tiles/pmtiles/pg_parcels_251001_m07.pmtile';
    this.sourceLayer = 'default'; // Default source layer, will be updated from metadata
  }

  /**
   * Add the parcels-m07 layer to a MapLibre GL map
  ...
  async addToMap(map) {
    try {
      ...
      map.on('sourcedata', e => {
        if (e.sourceId === 'parcels-m07' && e.isSourceLoaded) {
          // _onsole.log('✓ Parcels M07 PMTiles source loaded successfully');
        }
      });

      map.on('error', e => {
        if (e.error && e.error.message.includes('parcels-m07')) {
          console.error(' Parcels M07 layer error:', e.error);
        }
      });
      ...
      if (toggleButton) {
        ...
        if (titleSpan) {
          titleSpan.textContent = 'IMPIANTI ACQUE';
        }
      ...
      // _onsole.log('✓ Parcels M07 PMTiles layer added successfully');
    } catch (error) {
      console.error(' Failed to add parcels-m07 PMTiles layer:', error);
    }
  }
  ...
  setVisibility(map, visible) {
    ...
    if (map.getLayer('parcels-m07-fill')) {
      map.setLayoutProperty('parcels-m07-fill', 'visibility', visibility);
    }
    if (map.getLayer('parcels-m07-highlight')) {
      map.setLayoutProperty('parcels-m07-highlight', 'visibility', visibility);
    }
    if (map.getLayer('parcels-m07-selected')) {
      map.setLayoutProperty('parcels-m07-selected', 'visibility', visibility);
    }
  }

  /**
   * Show the parcels-m07 layers
   * @param {Map} map - MapLibre GL map instance
   */
  showLayers(map) {
  ...
  /**
   * Check if the parcels-m07 layers are currently visible
   * @param {Map} map - MapLibre GL map instance
   * @returns {boolean} True if layers are visible
   */
  isVisible(map) {
    const fillLayer = map.getLayer('parcels-m07-fill');
    if (fillLayer) {
  ...
  setLayerColor(newColor, map = null) {
    ...
    if (map && map.getLayer('parcels-m07-fill')) {
      map.setPaintProperty('parcels-m07-fill', 'fill-color', this.layerColor);
    }
  }
