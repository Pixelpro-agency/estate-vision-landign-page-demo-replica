/**
 * Parcels M03 Layer Configuration
...
export class ParcelsM03Layer {
  constructor() {
    this.layerName = 'parcels-m03';
    this.layerState = true; // Default visible state
    this.layerInToc = true; // Include in Table of Contents
    this.layerColor = '#8B4513'; // Saddle brown color for industrial facilities/quarries/mines
    this.pmtilesUrl =
      'https://storage.googleapis.com/space-neural-02/tiles/pmtiles/pg_parcels_251001_m03.pmtile';
    this.sourceLayer = 'default'; // Default source layer, will be updated from metadata
  }

  /**
   * Add the parcels-m03 layer to a MapLibre GL map
  ...
  async addToMap(map) {
    try {
      ...
      map.on('sourcedata', e => {
        if (e.sourceId === 'parcels-m03' && e.isSourceLoaded) {
          // _onsole.log('✓ Parcels M03 PMTiles source loaded successfully');
        }
      });

      map.on('error', e => {
        if (e.error && e.error.message.includes('parcels-m03')) {
          console.error(' Parcels M03 layer error:', e.error);
        }
      });
      ...
      if (toggleButton) {
        ...
        if (titleSpan) {
          titleSpan.textContent = 'IMPIANTI CAVE/MINIERE';
        }
      ...
      // _onsole.log('✓ Parcels M03 PMTiles layer added successfully');
    } catch (error) {
      console.error(' Failed to add parcels-m03 PMTiles layer:', error);
    }
  }
  ...
  setVisibility(map, visible) {
    ...
    if (map.getLayer('parcels-m03-fill')) {
      map.setLayoutProperty('parcels-m03-fill', 'visibility', visibility);
    }
    if (map.getLayer('parcels-m03-highlight')) {
      map.setLayoutProperty('parcels-m03-highlight', 'visibility', visibility);
    }
    if (map.getLayer('parcels-m03-selected')) {
      map.setLayoutProperty('parcels-m03-selected', 'visibility', visibility);
    }
  }

  /**
   * Show the parcels-m03 layers
   * @param {Map} map - MapLibre GL map instance
   */
  showLayers(map) {
  ...
  /**
   * Check if the parcels-m03 layers are currently visible
   * @param {Map} map - MapLibre GL map instance
   * @returns {boolean} True if layers are visible
   */
  isVisible(map) {
    const fillLayer = map.getLayer('parcels-m03-fill');
    if (fillLayer) {
   ...
   * @param {LeftSidebar} _sidebar - The sidebar instance (unused)
   * @returns {boolean} True if layer is enabled in sidebar
   */
  getLayerState(_sidebar) {
   ...
   * @param {Map} map - MapLibre GL map instance
   * @param {LayerControls} layerControls - The layer controls instance
   */
  setLayerState(newState, map, layerControls) {
    this.layerState = newState;
    this.setVisibility(map, newState);
    if (
      layerControls &&
      typeof layerControls.updateLayerToggleButton === 'function'
    ) {
      layerControls.updateLayerToggleButton(this.layerName, newState);
    }
  }
   ...
   * @param {LayerControls} layerControls - The layer controls instance
   */
  toggleLayerState(map, layerControls) {
    this.setLayerState(!this.layerState, map, layerControls);
  }
  ...
  setLayerColor(newColor, map = null) {
    ...
    if (map && map.getLayer('parcels-m03-fill')) {
      map.setPaintProperty('parcels-m03-fill', 'fill-color', this.layerColor);
    }
  }
