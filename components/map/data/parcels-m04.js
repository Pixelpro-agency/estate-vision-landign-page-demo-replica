/**
 * Parcels M04 Layer Configuration
...
export class ParcelsM04Layer {
  constructor() {
    this.layerName = 'parcels-m04';
    this.layerState = true; // Default visible state
    this.layerInToc = true; // Include in Table of Contents
    this.layerColor = '#FF4500'; // Orange red color for transportation
    this.pmtilesUrl =
      'https://storage.googleapis.com/space-neural-02/tiles/pmtiles/pg_parcels_251001_m04.pmtile';
    this.sourceLayer = 'default'; // Default source layer, will be updated from metadata
  }

  /**
   * Add the parcels-m04 layer to a MapLibre GL map
  ...
  async addToMap(map) {
    try {
      ...
      map.on('sourcedata', e => {
        if (e.sourceId === 'parcels-m04' && e.isSourceLoaded) {
          // _onsole.log('✓ Parcels M04 PMTiles source loaded successfully');
        }
      });

      map.on('error', e => {
        if (e.error && e.error.message.includes('parcels-m04')) {
          console.error(' Parcels M04 layer error:', e.error);
        }
      });
      ...
      if (toggleButton) {
        ...
        if (titleSpan) {
          titleSpan.textContent = 'TRASPORTI';
        }
      ...
      // _onsole.log('✓ Parcels M04 PMTiles layer added successfully');
    } catch (error) {
      console.error(' Failed to add parcels-m04 PMTiles layer:', error);
    }
  }
  ...
  setVisibility(map, visible) {
    ...
    if (map.getLayer('parcels-m04-fill')) {
      map.setLayoutProperty('parcels-m04-fill', 'visibility', visibility);
    }
    if (map.getLayer('parcels-m04-highlight')) {
      map.setLayoutProperty('parcels-m04-highlight', 'visibility', visibility);
    }
    if (map.getLayer('parcels-m04-selected')) {
      map.setLayoutProperty('parcels-m04-selected', 'visibility', visibility);
    }
  }

  /**
   * Show the parcels-m04 layers
   * @param {Map} map - MapLibre GL map instance
   */
  showLayers(map) {
  ...
  /**
   * Check if the parcels-m04 layers are currently visible
   * @param {Map} map - MapLibre GL map instance
   * @returns {boolean} True if layers are visible
   */
  isVisible(map) {
    const fillLayer = map.getLayer('parcels-m04-fill');
    if (fillLayer) {
  ...
  setLayerColor(newColor, map = null) {
    ...
    if (map && map.getLayer('parcels-m04-fill')) {
      map.setPaintProperty('parcels-m04-fill', 'fill-color', this.layerColor);
    }
  }
