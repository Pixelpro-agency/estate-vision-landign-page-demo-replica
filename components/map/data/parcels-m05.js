/**
 * Parcels M05 Layer Configuration
...
export class ParcelsM05Layer {
  constructor() {
    this.layerName = 'parcels-m05';
    this.layerState = true; // Default visible state
    this.layerInToc = true; // Include in Table of Contents
    this.layerColor = '#FF69B4'; // Hot pink color for tourism
    this.pmtilesUrl =
      'https://storage.googleapis.com/space-neural-02/tiles/pmtiles/pg_parcels_251001_m05.pmtile';
    this.sourceLayer = 'default'; // Default source layer, will be updated from metadata
  }

  /**
   * Add the parcels-m05 layer to a MapLibre GL map
  ...
  async addToMap(map) {
    try {
      ...
      map.on('sourcedata', e => {
        if (e.sourceId === 'parcels-m05' && e.isSourceLoaded) {
          // _onsole.log('✓ Parcels M05 PMTiles source loaded successfully');
        }
      });

      map.on('error', e => {
        if (e.error && e.error.message.includes('parcels-m05')) {
          console.error(' Parcels M05 layer error:', e.error);
        }
      });
      ...
      // _onsole.log('✓ Parcels M05 PMTiles layer added successfully');
    } catch (error) {
      console.error(' Failed to add parcels-m05 PMTiles layer:', error);
    }
  }
  ...
  setVisibility(map, visible) {
    ...
    if (map.getLayer('parcels-m05-fill')) {
      map.setLayoutProperty('parcels-m05-fill', 'visibility', visibility);
    }
    if (map.getLayer('parcels-m05-highlight')) {
      map.setLayoutProperty('parcels-m05-highlight', 'visibility', visibility);
    }
    if (map.getLayer('parcels-m05-selected')) {
      map.setLayoutProperty('parcels-m05-selected', 'visibility', visibility);
    }
  }

  /**
   * Show the parcels-m05 layers
   * @param {Map} map - MapLibre GL map instance
   */
  showLayers(map) {
  ...
  /**
   * Check if the parcels-m05 layers are currently visible
   * @param {Map} map - MapLibre GL map instance
   * @returns {boolean} True if layers are visible
   */
  isVisible(map) {
    const fillLayer = map.getLayer('parcels-m05-fill');
    if (fillLayer) {
  ...
  setLayerColor(newColor, map = null) {
    ...
    if (map && map.getLayer('parcels-m05-fill')) {
      map.setPaintProperty('parcels-m05-fill', 'fill-color', this.layerColor);
    }
  }
